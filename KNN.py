...
# --- 1. Função para extrair características (GEOMÉTRICOS E COR ANEL-NUCLEO) ---
...
# --- 3. Função para treinar o modelo (KNN) ---
def treinar_modelo():
    dataset_path = "all"
    X, y = [], []
...
# --- 4. Lógica para teste individual ---
...
def visualizar_filtros():
    ...
    try:
        ...
        if contornos:
            ...
            raio = int(raio)
            raio_nucleo = int(raio * 0.6)
            mascara = np.zeros(imagem_original.shape[:2], dtype=np.uint8)
            cv2.drawContours(mascara, [cnt], -1, 255, -1)
            mascara_nucleo = np.zeros_like(mascara)
            cv2.circle(mascara_nucleo, centro, raio_nucleo, 255, -1)
            mascara_anel = cv2.subtract(mascara, mascara_nucleo)
            img_nucleo = cv2.bitwise_and(imagem_original, imagem_original, mask=mascara_nucleo)
            img_anel = cv2.bitwise_and(imagem_original, imagem_original, mask=mascara_anel)
            ...
            img_nucleo = np.zeros_like(imagem_original)
            img_anel = np.zeros_like(imagem_original)
        ...
        cv2.imshow("5 - Núcleo da Moeda", img_nucleo)
        cv2.imshow("6 - Anel da Moeda", img_anel)
