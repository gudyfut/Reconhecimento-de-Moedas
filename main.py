...
# --- 1. Função para extrair características (SIMPLIFICADA) ---
def extrair_caracteristicas(contorno, imagem_bgr):
    # --- Atributos geométricos ---
    area = cv2.contourArea(contorno)
    perimetro = cv2.arcLength(contorno, True)
    # Circularidade é mantida como um atributo útil
    ...
    # O vetor de características agora é mais simples
    return [area, perimetro, circularidade, *media_cor]
...
def visualizar_filtros():
    ...
    try:
        ...
        if contornos:
            ...
            print(f"1. Área: {carac[0]:.2f} pixels")
            print(f"2. Perímetro: {carac[1]:.2f} pixels")
            print(f"3. Circularidade: {carac[2]:.4f}")
            print(f"4. Cor Média (B, G, R): ({carac[3]:.2f}, {carac[4]:.2f}, {carac[5]:.2f})")
            print("------------------------------------")
